...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnColostrum = document.getElementById('btn-colostrum');
    const btnClean = document.getElementById('btn-clean');
    const btnVet = document.getElementById('btn-vet');
    ...
    function addSummaryRow(trial) {
        ...
        tr.innerHTML = `
            ...
            <td>${trial.colostrum ? 'Verildi' : 'Hayır'}</td>
            <td>${trial.cleanCount}</td>
            <td>${trial.vetCount}</td>
    ...
    function disableActions() {
        btnColostrum.disabled = true;
        btnClean.disabled = true;
        btnVet.disabled = true;
    ...
    btnStart.addEventListener('click', () => {
        ...
        if (btnStart.innerText === "Yeniden Başlat" || btnStart.innerText === "Simülasyona Devam Et") {
            if (btnStart.innerText === "Yeniden Başlat") {
                ...
                engine.init(workerSlider.value, ventSlider.value, hygieneSlider.value, budgetInput.value, barnSelect.value, breedSelect.value);
            }
        } else {
            currentLogs = [];
            lastEventReason = "Durduruldu";
        ...
        btnColostrum.disabled = false;
        btnClean.disabled = false;
        btnVet.disabled = false;
    ...
    btnColostrum.addEventListener('click', () => {
        if (engine.feedColostrum()) {
            logMessage("Aksiyon: Kolostrum verildi! IgG seviyesi desteklendi.", "success");
            btnColostrum.disabled = true; 
        }
    });
    ...
    btnClean.addEventListener('click', () => {
        if (engine.cleanBarn()) {
            logMessage("Aksiyon: Ahır pırıl pırıl oldu (Masraf: 50₺). Nöbetçi bakıcı biraz söylendi ama işi halletti.", "system");
        } else {
            logMessage("Hata: İçeride zinde bakıcı kalmadı, herkes yerlerde sürünüyor! Temizlik yapılamaz.", "warning");
        }
    });

    btnVet.addEventListener('click', () => {
        if (engine.callVet()) {
            logMessage("Aksiyon: Veteriner sirenlerle geldi (-500₺). Hayvana sihirli iğne yapıldı (+40 Sağlık).", "success");
        } else {
            logMessage("Hata: Ya paranız bitti ya da veteriner çağırmak için çok geç kaldınız!", "danger");
        }
    });
    ...
    btnSave.addEventListener('click', () => {
        ...
        let trialObject = {
            ...
            colostrum: engine.calf ? engine.calf.colostrumFed : false,
            cleanCount: engine.cleanCount,
            vetCount: engine.vetCount,
    ...
    btnExportCSV.addEventListener('click', () => {
        ...
        let csvContent = "Deneme No,Irk,Ahir Tipi,Bakici Sayisi,Havalandirma (%),Kolostrum,Temizlik Sayisi,Vet Cagrisi,Son Saat,Final Bagisiklik,Final Saglik,Ilk Bilanco (TL),Final Bilanco (TL),Sonuc\n";

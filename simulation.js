...
class EnvironmentAgent {
    constructor(ventilationPower, barnType, breed, skipFetch = false) {
...
class SimulationEngine {
    ...
    init(workerCount, ventilation, hygiene, budget, barnType, breed, colostrumTime, skipFetch = false) {
        this.time = 0;
        this.history = [];
        this.budget = parseInt(budget);
        this.barnType = barnType;
        this.colostrumTargetTime = parseInt(colostrumTime);
        ...
        this.env = new EnvironmentAgent(ventilation, barnType, breed, this.skipFetch);
    }
    ...
    tick() {
        ...
        let colostrumGivenNow = false;
        if (this.time === this.colostrumTargetTime && !this.calf.colostrumFed) {
            this.feedColostrum();
            colostrumGivenNow = true;
        }
        
        // Otomatik Temizlik: Amonyak belli bir seviyeyi aşarsa en dinç işçi temizler
        if (this.env.ammonia > 8) {
            let bestWorker = [...this.workers].sort((a,b) => a.fatigue - b.fatigue)[0];
            if (bestWorker && bestWorker.fatigue < 80) {
                this.budget -= 50; // Temizlik masrafı
                bestWorker.cleanBarn(this.env);
            }
        }
        ...
        let currentData = {
            ...
            colostrumGivenNow: colostrumGivenNow
        };
        ...
        } else if (this.time >= 72) {
            this.stop();
            this.notify({ event: "SUCCESS", msg: "TEBRİKLER! 3 gün dayandınız. Yılın Ziraat Ödülü size gidiyor!" });
        }
    }
